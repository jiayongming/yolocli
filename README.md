...
- 🤸 **姿势估计 (Pose Estimation)**: 检测人体/物体并标注关键点位置
...
## 🆕 最新更新 (v1.0.0 - 2026-01-13)

### ⚙️ 交互式训练高级配置 🎨
- ✅ **数据集智能选择** 🆕：从 `datasets` 目录交互式选择数据集，无需记住路径
...
YOLO CLI支持四种计算机视觉任务，每种任务有不同的应用场景和数据格式要求：
...
### 4. 姿势估计 (Pose Estimation)
- **功能**: 检测人体/物体并标注关键点位置（如骨架关节点）
- **输出**: 边界框 + 关键点坐标 + 关键点置信度 + 可见性标志
- **应用**: 人体姿态分析、动作识别、运动追踪、健身指导等
- **数据格式**: YOLO Pose格式（每行：`class_id x y w h kp1_x kp1_y kp1_v kp2_x kp2_y kp2_v ...`）
- **模型后缀**: `yolo11s-pose.pt`
- **关键点配置**: 支持COCO 17关键点或自定义关键点数量
...
4. **选择数据集来源** 🔥：
   - **从 datasets 目录选择（推荐）** 🆕：
     - 自动扫描 `datasets/` 目录下的所有数据集
     - 选择一个已准备好的数据集（已划分、已合并或已过滤的数据集）
     - 自动找到并使用数据集的 `data.yaml` 或 `dataset.yaml` 配置文件
   - **手动输入路径**：
     - 输入自定义的数据集配置文件路径
5. **确认并开始训练**：
   - 显示完整配置摘要（模型、数据集、训练参数等）
   - 确认后开始训练

优势：
- ✅ 无需记住数据集路径，直接从列表选择
- ✅ 显示数据集配置文件状态（有/无 `data.yaml`）
- ✅ 支持使用 `data merge`、`data filter`、`data move-dataset` 等命令准备的数据集
- ✅ 与数据处理工作流无缝衔接
...
python yolo_cli.py data verify [OPTIONS]
  --path TEXT        数据集路径（可选，不指定则从 datasets 目录中选择）🆕
  --task TEXT        任务类型 (detect/segment/pose，默认: detect)

使用方式 🆕：
  # 交互式选择数据集（支持多选，一次验证多个数据集）🔥
  python yolo_cli.py data verify --task detect
  
  # 或手动指定路径（单个数据集）
  python yolo_cli.py data verify --path datasets/mydata --task detect
...
数据集审计功能 🔥 NEW：
  ✓ 标签格式分布统计（自动检测 detect/segment/pose/empty 格式）
  ✓ 混合格式检测（警告格式不一致的标签文件）
  ✓ 空标签文件统计（负样本检测）
  ✓ 孤儿文件检测（图片无标签 / 标签无图片）
  ✓ 格式不匹配示例展示
...
交互式选择示例（支持多选）🆕:
  ...
  ℹ 已选择 3 个数据集: 背包, 扳手, 烟头
  
  ═══════════════════════════════════════════════════
  
  ──────────────────── 验证数据集 [1/3]: 背包 ────────────────────
  ℹ 数据集路径: /path/to/datasets/背包
  ...
  
  ═══════════════════════════════════════════════════
  
  ──────────────────── 验证数据集 [2/3]: 扳手 ────────────────────
  ℹ 数据集路径: /path/to/datasets/扳手
  ...
  
  ═══════════════════════════════════════════════════
  
  ──────────────────── 验证数据集 [3/3]: 烟头 ────────────────────
  ℹ 数据集路径: /path/to/datasets/烟头
  ...
...
示例输出:
  ...
  │ 格式类型           │ 数量   │ 占比   │
  ├────────────────────┼────────┼────────┤
  │ ✓ DETECT (预期)    │ 10913  │ 88.4%  │
  │ SEGMENT            │ 1149   │ 9.3%   │
  │ EMPTY              │ 294    │ 2.4%   │
  ╰────────────────────┴────────┴────────╯
  
  ⚠️  检测到 1149 个标签与预期格式不匹配!
  格式不匹配详情:
    • SEGMENT 格式: 1149 个文件
  
  示例文件（前10个）:
    • train/image001.txt → SEGMENT
    • train/image023.txt → SEGMENT
    ...
  
  💡 建议:
    • 使用 'data format-convert --from segment --to detect' 转换格式
  
  📭 发现 294 个空标签文件（负样本）
  
  🔍 孤儿文件检测:
    • 15 个图片缺少对应标签
      - train: image_missing_label_001
      - train: image_missing_label_002
      ...
  ...
  详细错误信息（前20个）:
    • train: 缺少标签 - image_missing_label_001
    ...
...
使用方式 🆕：
  # 交互式选择数据集（支持多选，一次统计多个数据集）🔥
  python yolo_cli.py data stats --detailed --task detect
  
  # 或手动指定路径（单个数据集）
  python yolo_cli.py data stats --path datasets/mydata --detailed --task detect
...
示例输出:
  ╭────────┬───────────┬────────┬──────────┬────────┬──────────╮
  │ 类别ID │ 类别名称  │ 标注数 │ 标注占比 │ 样本数 │ 样本占比 │
  ├────────┼───────────┼────────┼──────────┼────────┼──────────┤
  │ 0      │ person    │ 16420  │ 55.0%    │ 8500   │ 68.9%    │
  │ 1      │ car       │ 186    │ 0.6%     │ 150    │ 1.2%     │
  ╰────────┴───────────┴────────┴──────────┴────────┴──────────╯
  
  说明: person类有16420个标注（占总标注的55.0%）
        分布在8500张图片中（占总样本的68.9%）
        → 平均每张图片约1.93个person标注
  
  对比分析:
        person: 样本占比(68.9%) > 标注占比(55.0%)
        → 该类别在图片中分布广泛但密度相对较低
        
        car: 样本占比(1.2%) > 标注占比(0.6%)  
        → 该类别相对稀少且密度低
